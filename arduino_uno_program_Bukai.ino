...
// Klasifikacia farieb podla RGB, normalizacia podla C a vypocet HUE
String classifyColor(uint16_t r, uint16_t g, uint16_t b, uint16_t c) {
  ...
  float hue = 0;
  if (delta > 0) {
    if (maxVal == rNorm) hue = 60.0 * fmod(((gNorm - bNorm) / delta), 6);
    else if (maxVal == gNorm) hue = 60.0 * (((bNorm - rNorm) / delta) + 2);
    else hue = 60.0 * (((rNorm - gNorm) / delta) + 4);
  }
...
void wiggleAtDropPosition(int center, int range = 5, int repetitions = 3) {
...
// Pokrocili delay s moznostou prerusenia - potrebna pri zastaveni systemu s vypisom na serial monitor
...
// Hlavna slucka - vypis do serial monitoru na diagnostiku
void loop() {
  if (Serial.available()) {
...
// urcenie farby z merani - vypis do serial monitoru na diagnostiku
...
// otocenia serva na triedenie - vypis do serial monitoru na diagnostiku
...
// vratenia serva na zaciatocnu poziciu
