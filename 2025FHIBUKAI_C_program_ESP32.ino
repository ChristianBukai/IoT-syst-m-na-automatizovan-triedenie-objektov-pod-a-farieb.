...
// Odosielanie mapovania na arduino
...
// Inicializacia servera so vsetkymi prvkami
void setup() {
  Serial.begin(115200);
  Serial2.begin(9600, SERIAL_8N1, RXD2, TXD2);

  // pripojenie na WIFI siet
  ...
  // ziskanie IP adresy a urcenie aktualneho casoveho pasma
  Serial.println("\nWiFi connected.");
  ...
  // zobrazenie poctu vsetkych farieb
  server.on("/tally", []() {
    String html = "<ul>";
    const char* colorNames[5] = {"YELLOW", "ORANGE", "RED", "PURPLE", "GREEN"};
    for (int i = 0; i < 5; i++) {
  ...
  // nastavenie automatickeho zastavenia
...
void loop() {
  ...
  if (Serial2.available()) {
    ...
    // ziskanie farby zo spravy vo formate "COLOR-POSITION"
    if (msg.length() > 0 && colorCount < 100) {
      int sep = msg.indexOf('-');
      String color = sep != -1 ? msg.substring(0, sep) : msg;
      color.toUpperCase();

      // priradenie indexu farby podla nazvu na zaklade poradia v poli farieb.
      int colorIndex = -1;
      ...
      if (pos >= 1 && pos <= 5) {
        ...
        // zapis novej instancie triedenia
